...
struct ImGuiTextBuffer;             // Helper to hold and append into a text buffer (~std::string builder)
...
typedef unsigned int ImGuiID;       // Unique ID used by widgets (typically hashed from a stack of std::string)
...
// 2D std::vector (often used to store positions, sizes, etc.)
struct ImVec2
{
    float     x, y;
    ImVec2()  { x = y = 0.0f; }
...
// 4D std::vector (often used to store floating-point colors)
struct ImVec4
{
    float     x, y, z, w;
    ImVec4()  { x = y = z = w = 0.0f; }
...
namespace ImGui
{
    ...
    IMGUI_API const char*   GetVersion();                               // get the compiled version std::string e.g. "1.23"
    ...
    // In this header file we use the "label"/"name" terminology to denote a std::string that will be displayed and used as an ID,
    // whereas "str_id" denote a std::string that is only used as an ID and not aimed to be displayed.
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL);                // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated std::string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
    ...
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items = -1);      // Separate items with \0 within a std::string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    ...
    // Adjust format std::string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    ...
    // If you want to use InputText() with a dynamic std::string type such as std::string or your own, see misc/cpp/imgui_stdlib.h
    ...
    IMGUI_API bool          TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(2);   // helper variation to completely decorelate the id from the displayed std::string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
    ...
    IMGUI_API bool          SetDragDropPayload(const char* type, const void* data, size_t size, ImGuiCond cond = 0);// type is a user defined std::string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui.
    ...
    // Inputs Utilities
    IMGUI_API int           GetKeyIndex(ImGuiKey imgui_key);                                    // std::map ImGuiKey_* values into user's key index. == io.KeyMap[key]
    ...
    IMGUI_API const char*   SaveIniSettingsToMemory(size_t* out_ini_size = NULL);               // return a zero-terminated std::string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackResize      = 1 << 18,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the std::string to grow. Notify when the std::string wants to be resized (for std::string types which hold a cache of their Size). You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of using this)
    // [Internal]
...
struct ImGuiIO
{
    ...
    IMGUI_API void AddInputCharactersUTF8(const char* utf8_chars);      // Add new characters into InputCharacters[] from an UTF-8 std::string
...
// - ImGuiInputTextFlags_CallbackResize:      Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the std::string to grow. 
struct ImGuiInputTextCallbackData
{
    ...
    int                 BufTextLen;     // Text length (in bytes)               // Read-write   // [Resize,Completion,History,Always] Exclude zero-terminator storage. In C land: == strlen(some_text), in C++ land: std::string.length()
    int                 BufSize;        // Buffer size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In C land == ARRAYSIZE(my_char_array), in C++ land: std::string.capacity()+1
...
struct ImGuiPayload
{
    ...
    char            DataType[32+1];     // Data type tag (short user-supplied std::string, 32 characters max)
...
struct ImFontAtlas
{
    ...
    // NB: Make sure that your std::string are UTF-8 and NOT in your local code page. In C++11, you can create UTF-8 std::string literal using the u8"Hello world" syntax. See FAQ for details.
    ...
    struct GlyphRangesBuilder
    {
        ...
        IMGUI_API void AddText(const char* text, const char* text_end = NULL);      // Add std::string (each character of the UTF-8 std::string are added)
    ...
    struct CustomRect
    {
        unsigned int    ID;             // Input    // User ID. Use <0x10000 to std::map into a font glyph, >=0x10000 for other/internal/custom texture data.
    ...
    IMGUI_API int       AddCustomRectFontGlyph(ImFont* font, ImWchar id, int width, int height, float advance_x, const ImVec2& offset = ImVec2(0,0));   // Id needs to be < 0x10000 to register a rectangle to std::map into a specific font.
