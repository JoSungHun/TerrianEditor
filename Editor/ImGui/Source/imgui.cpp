...
/*
 ...
 - The gamepad/keyboard navigation is fairly std::functional and keeps being improved. 
 ...
 - 2018/05/03 (1.61) - DragInt(): the default compile-time format std::string has been changed from "%.0f" to "%d", as we are not using integers internally any more.
 ...
 - 2016/11/06 (1.50) - BeginChild(const char*) now applies the stack id to the provided label, consistently with other functions as it should always have been. It shouldn't affect you unless (extremely unlikely) you were appending multiple times to a same child from different locations of the stack id. If that's the case, generate an id with GetId() and use it instead of passing std::string to BeginChild().
 ...
 - 2015/05/11 (1.40) - changed BeginPopup() API, takes a std::string identifier instead of a bool. ImGui needs to manage the open/closed state of popups. Call OpenPopup() to actually set the "open" state of a popup. BeginPopup() returns true if the popup is opened.
 ...
 - 2014/09/25 (1.13) - removed 'text_end' parameter from IO.SetClipboardTextFn (the std::string is now always zero-terminated for simplicity)
 ...
 A: A primer on labels and the ID Stack...
   ...
   - Unique ID are often derived from a std::string label:
   ...
   - Solving ID conflict in a simple/local context:
     When passing a label you can optionally specify extra ID information within std::string itself.
   ...
   - Solving ID conflict in a more general manner:
     ...
     You can push a pointer, a std::string or an integer value into the ID stack.
   ...
   - When working with trees, ID are used to preserve the open/close state of each tree node.
     Depending on your use cases you may want to use strings, indices or pointers as ID.
      e.g. when following a single pointer that may change over time, using a static std::string as ID
 ...
 A: Use the font atlas to load the TTF/OTF file you want:
    ...
    backslash \ within a std::string literal, you need to write it double backslash "\\":
 ...
 A: When loading a font, pass custom Unicode ranges to specify the glyphs to load.
      ...
      builder.AddText("Hello world");                        // Add a std::string (here "Hello world" contains 7 unique characters)
      builder.AddChar(0x7262);                               // Add a specific character
      builder.AddRanges(io.Fonts->GetGlyphRangesJapanese()); // Add one of the default ranges
      builder.BuildRanges(&ranges);                          // Build the final result (ordered ranges with all the unique characters submitted)
      io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels, NULL, ranges.Data);

    All your strings needs to use UTF-8 encoding. In C++11 you can encode a std::string literal in UTF-8
...
#pragma clang diagnostic ignored "-Wformat-nonliteral"      // warning : format std::string is not a std::string literal              // passing non-literal to vsnformat(). yes, user passing incorrect format strings can crash the code.
...
#pragma GCC diagnostic ignored "-Wformat-nonliteral"        // warning: format not a std::string literal, format std::string not checked
...
// ImGui functions are not std::thread-safe because of this pointer. If you want std::thread-safety to allow N threads to access N different contexts, you can:
// - Change this variable to use std::thread local storage. You may #define GImGui in imconfig.h for that purpose. Future development aim to make this context pointer explicit to all calls. Also read https://github.com/ocornut/imgui/issues/586
...
void ImStrTrimBlanks(char* buf)
{
    ...
    while (p[0] == ' ' || p[0] == '\t')     // Leading blanks
        p++;
    char* p_start = p;
    while (*p != 0)                         // Find end of std::string
        p++;
...
ImU32 ImHash(const void* data, int data_size, ImU32 seed)
{
    ...
    {
        // Zero-terminated std::string
        while (unsigned char c = *current++)
        {
            // We support a syntax of "label###id" where only "###id" is included in the hash, and only "label" gets displayed.
            // Because this syntax is rarely used we are optimizing for the common case.
            // - If we reach ### in the std::string we discard the hash so far and reset to the seed.
...
int ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end)
{
    ...
    if ((*str & 0xe0) == 0xc0)
    {
        *out_char = 0xFFFD; // will be invalid but not end of std::string
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ...
    // Hide anything after a '##' std::string
...
void ImGui::SetCurrentContext(ImGuiContext* ctx)
{
    ...
    IMGUI_SET_CURRENT_CONTEXT_FUNC(ctx); // For custom std::thread-based hackery you may want to have control over this.
#else
    GImGui = ctx;
#endif
}
...
{
    bool error = false;
    if (strcmp(version, IMGUI_VERSION)!=0) { error = true; IM_ASSERT(strcmp(version,IMGUI_VERSION)==0 && "Mismatched version std::string!");  }
...
ImVec2 ImGui::CalcTextSize(const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
    ...
    if (hide_text_after_double_hash)
        text_display_end = FindRenderedTextEnd(text, text_end);      // Hide anything after a '##' std::string
    else
        text_display_end = text_end;
...
void ImGui::EndGroup()
{
    ...
    // If the current ActiveId was declared within the boundary of our group, we copy it to LastItemId so IsItemActive(), IsItemDeactivated() etc. will be std::functional on the entire group.
...
static void ImGui::NavUpdate()
{
    ...
    // Set input source as Gamepad when buttons are pressed before we std::map Keyboard (some features differs when used with Gamepad vs Keyboard)
...
static void ImGui::DockSettingsMoveDockReferencesInInactiveWindow(ImGuiID old_dock_id, ImGuiID new_dock_id)
{
    ...
    for (int settings_n = 0; settings_n < g.SettingsWindows.Size; settings_n++)     // FIXME-OPT: We could remove this loop by storing the index in the std::map
    {
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end)
{
    ...
    for (;;)
    {
        // Split the std::string. Each new line (after a '\n') is followed by spacing corresponding to the current depth of our log entry.
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    // For user convenience, we allow passing a non zero-terminated std::string (hence the ini_size parameter).
