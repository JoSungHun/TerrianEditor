...
// likely going to be stored outside your functions.asfsddddddddddd
...
#pragma clang diagnostic ignored "-Wformat-security"            // warning : warning: format std::string is not a std::string literal
...
#pragma GCC diagnostic ignored "-Wformat-security"              // warning : format std::string is not a std::string literal (potentially insecure)
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
...

        {
            ...
            ImGui::SameLine(); ShowHelpMarker("USER:\nHold SHIFT or use mouse to select text.\n" "CTRL+Left/Right to word jump.\n" "CTRL+A or double-click to select all.\n" "CTRL+X,CTRL+C,CTRL+V clipboard.\n" "CTRL+Z,CTRL+Y undo/redo.\n" "ESCAPE to revert.\n\nPROGRAMMER:\nYou can use the ImGuiInputTextFlags_CallbackResize facility if you need to wire InputText() to a dynamic std::string type. See misc/cpp/imgui_stdlib.h for an example (this is not demonstrated in imgui_demo.cpp).");
    ...
    if (ImGui::TreeNode("Text"))
    {
        ...
        if (ImGui::TreeNode("UTF-8 Text"))
        {
            ...
            ImGui::Text("Hiragana: \xe3\x81\x8b\xe3\x81\x8d\xe3\x81\x8f\xe3\x81\x91\xe3\x81\x93 (kakikukeko)"); // Normally we would use u8"blah blah" with the proper characters directly in the std::string.
    ...
    if (ImGui::TreeNode("Combo"))
    {
        ...
        // Simplified one-liner Combo() API, using values packed in a single constant std::string
    ...
    if (ImGui::TreeNode("Multi-line Text Input"))
    {
        ...
        ShowHelpMarker("You can use the ImGuiInputTextFlags_CallbackResize facility if you need to wire InputTextMultiline() to a dynamic std::string type. See misc/cpp/imgui_stdlib.h for an example. (This is not demonstrated in imgui_demo.cpp)");
...
static void ShowDemoWindowPopups()
{
    ...
    if (ImGui::TreeNode("Popups"))
    {
        ...
        // (If you want to show the current selection inside the Button itself, you may want to build a std::string using the "###" operator to preserve a constant ID with a variable label)
...
// Here we use the simplified Combo() api that packs items into a single literal std::string. Useful for quick combo boxes where the choices are known locally.
                    ...
                    for (int base = 0; base < 0x10000; base += 256)
                    {
                        ...
                        if (count > 0 && ImGui::TreeNode((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base+255, count, count > 1 ? "glyphs" : "glyph"))
                        {
                            ...
                            for (int n = 0; n < 256; n++)
                            {
                                ...
                                if (glyph)
                                    font->RenderChar(draw_list, cell_size, cell_p1, ImGui::GetColorU32(ImGuiCol_Text), (ImWchar)(base+n)); // We use ImFont::RenderChar as a shortcut because we don't have UTF-8 conversion functions available to generate a std::string.
...
void ShowExampleAppDocuments(bool* p_open)
{
    ...
    if (opt_target == Target_Tab)
    {
        ...
        if (ImGui::BeginTabBar("##tabs", tab_bar_flags))
        {
            ...
            for (int doc_n = 0; doc_n < app.Documents.Size; doc_n++)
            {
                ...
                // Cancel attempt to close when unsaved add to save std::queue so we can display a popup.
    ...
    // Update closing std::queue
    static ImVector<MyDocument*> close_queue;
    if (close_queue.empty())
    {
        // Close std::queue is locked once we started a popup
        for (int doc_n = 0; doc_n < app.Documents.Size; doc_n++)
        {
            MyDocument* doc = &app.Documents[doc_n];
            if (doc->WantClose)
