...
// Then you can keep 'str_id' empty or the same for all your buttons (instead of creating a std::string based on a non-std::string id)
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    // Default to using texture ID as ID. User can still push std::string/integer prefixes.
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    // Default displaying the fraction as percentage std::string, but user can override it
    char overlay_buf[32];
    if (!overlay)
    {
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    // Call the getter to obtain the preview std::string which is a parameter to BeginCombo()
...
// Combo box helper allowing to pass all items in a single std::string literal holding multiple zero-terminated items "item1\0item2\0" 
...
// To honor backward compatibility we are rewriting the format std::string, unless IMGUI_DISABLE_OBSOLETE_FUNCTIONS is enabled. What could possibly go wrong?!
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f')
    {
        ...
        return g.TempBuffer;
#else
        IM_ASSERT(0 && "DragInt(): Invalid format std::string!"); // Old versions used a default parameter of "%.0f", please replace with e.g. "%d"
#endif
    }
    return fmt;
}
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type, TYPE v)
{
    ...
    if (fmt_start[0] != '%' || fmt_start[1] == '%') // Don't apply if the value is not visible in the format std::string
        return v;
    char v_str[64];
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiDragFlags flags)
{
    ...
    // Round to user desired precision based on format std::string
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    // Default format std::string when passing NULL
    // Patch old "%.0f" format std::string to use "%d", read function comments for more details.
...
// Extract the format out of a format std::string with leading or trailing decorations
...
// Parse display precision back from the display format std::string
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // Will copy result std::string if modified
