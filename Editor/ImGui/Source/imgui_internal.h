...
IMGUI_API int           ImTextCountUtf8BytesFromStr(const ImWchar* in_text, const ImWchar* in_text_end);                   // return number of bytes to express std::string in UTF-8
...
// FIXME: this is in development, not exposed/std::functional as a generic feature yet.
...
// 1D std::vector (this odd construct is used to facilitate the transition between 1D and 2D and maintenance of some patches)
struct ImVec1
{
    float   x;
    ImVec1()         { x = 0.0f; }
...
// 2D std::vector (half-size integer)
struct ImVec2ih
{
    short   x, y;
    ImVec2ih()                   { x = y = 0; }
